...
class AgentBackfillDemo:
    ...
    demo dataset.
    """
    def __init__(self, scene_dataset_json, creator):
    ...
    def backfill(self, original_dataset, split_type):
        ...
        for scene_idx, scene in enumerate(original_dataset.scenes):
            ...
            os.makedirs(os.path.join(output_scene_dirname, AGENT_FOLDER), exist_ok=True)
            ...
            save_pbobject_as_json(agent_tracks_pb2, os.path.join(output_scene_dirname, agent_tracks_filename))
            agent_pb2.agent_tracks_file = agent_tracks_filename

            # Save agents_slices file
            agents_slices_filename = os.path.join(
                AGENT_FOLDER,
                TRI_DGP_AGENTS_SLICES_JSON_NAME.format(slice_hash=generate_uid_from_pbobject(agent_tracks_pb2))
            )
            save_pbobject_as_json(agents_slices_pb2, os.path.join(output_scene_dirname, agents_slices_filename))
            ...
            scene_dirname = output_scene_dirname.replace(self.local_output_path + '/', '')
            agents_group_filename = os.path.join(
                scene_dirname, TRI_DGP_AGENTS_JSON_NAME.format(agent_hash=generate_uid_from_pbobject(agent_pb2))
            )
            ...
            # Populate ontologies
            os.makedirs(os.path.join(output_scene_dirname, FEATURE_ONTOLOGY_FOLDER), exist_ok=True)
            for feature_type, ontology_id in agent_pb2.feature_ontologies.items():
                ontology_filename = os.path.join(
                    output_scene_dirname, FEATURE_ONTOLOGY_FOLDER, "{}.json".format(ontology_id)
                )
                save_pbobject_as_json(self.ontologies[feature_type], ontology_filename)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--creator", help="Creator email", required=True)  #
    args, other_args = parser.parse_known_args()
    if args.verbose:
        logging.basicConfig(level=logging.INFO)

    dataset = AgentBackfillDemo(args.scene_dataset_json, args.creator)
    dataset.generate()
