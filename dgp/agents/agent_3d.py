...
class AgentSnapshot3DList(AgentSnapshotList):
    ...
    @classmethod
    def load(cls, agent_snapshots_3d, ontology, feature_ontology_table):
        ...
        Parameters
        ----------
        agent_snapshots_3d: dgp.proto.agent.AgentsSlice.agent_snapshots or dgp.proto.agent.AgentTrack.agent_snapshots
        ...
        """
        boxlist = []
        for agent_snapshot_3d in agent_snapshots_3d:
    ...
    def render(self, image, camera, line_thickness=2, font_scale=0.5):
        ...
        """
        if (
                not isinstance(image, np.ndarray) or image.dtype != np.uint8 or len(image.shape) != 3 or image.shape[
            2] != 3
        ):
