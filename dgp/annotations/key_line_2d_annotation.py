...
class KeyLine2DAnnotationList(Annotation):
    ...
    def xy(self):
        ...
        return np.array([point.xy for point in self.linelist], dtype=np.float32)
    ...
    def class_ids(self):
        ...
        """
        return np.array([point.class_id for point in self.linelist], dtype=np.int64)

    @property
    def attributes(self):
        """Return a list of dictionaries of attribut name to value."""
        return [point.attributes for point in self.linelist]

    @property
    def instance_ids(self):
        return np.array([point.instance_id for point in self.linelist], dtype=np.int64)
