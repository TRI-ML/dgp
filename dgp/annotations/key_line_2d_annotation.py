...
class KeyLine2DAnnotationList(Annotation):
    ...
    def load(cls, annotation_file, ontology):
        ...
        linelist = [
            KeyLine2D(
                line=np.float32([[vertex.point.x, vertex.point.y] for vertex in ann.vertices]),
        ...
        return KeyLine2DAnnotations(
            annotations=[
                KeyLine2DAnnotation(
                    ...
                    vertices=[
                        KeyPoint2D(
                            point=[line.x[j], line.y[j]],
                            class_id=line.class_id,
                            instance_id=line.instance_id,
                            color=line.color,
                            attributes=line.attributes
                        ).to_proto() for i, j in enumerate(line.x)
                    ],
                    attributes=line.attributes
    ...
    def xy(self):
        ...
        return np.array([line.xy for line in self.linelist], dtype=np.float32)
