...
from ouroboros.dgp import FEATURE_ONTOLOGY_FOLDER, ONTOLOGY_FOLDER
from ouroboros.dgp.agents import (AGENT_REGISTRY,
                                  AGENT_TYPE_TO_ANNOTATION_TYPE,
                                  ANNOTATION_TYPE_TO_AGENT_TYPE)
from ouroboros.dgp.annotations import ONTOLOGY_REGISTRY
from ouroboros.dgp.constants import (ALL_FEATURE_TYPES,
                                     ANNOTATION_TYPE_ID_TO_KEY,
                                     FEATURE_TYPE_ID_TO_KEY)
from ouroboros.dgp.datasets.synchronized_dataset import \
    SynchronizedSceneDataset
from ouroboros.dgp.features import FEATURE_ONTOLOGY_REGISTRY
from ouroboros.dgp.proto import dataset_pb2
from ouroboros.dgp.proto.agent_pb2 import AgentGroup as AgentGroupPb2
from ouroboros.dgp.proto.agent_pb2 import AgentsSlices as AgentsSlicesPb2
from ouroboros.dgp.proto.agent_pb2 import AgentTracks as AgentTracksPb2
from ouroboros.dgp.proto.dataset_pb2 import Agents as AgentsPb2
from ouroboros.dgp.utils.protobuf import open_pbobject
...
class AgentContainer:
    ...
    cache_dir = os.path.join(OUROBOROS_CACHE_DIR, f'dgp_diskcache_{RANDOM_STR}')
    ...
    def feature_ontology_files(self):
        ...
        feature_ontology_files = {
            FEATURE_TYPE_ID_TO_KEY[feature_id]: os.path.join(self.directory, FEATURE_ONTOLOGY_FOLDER,
                                                             "{}.json".format(f))
    ...
    def load_agent_data(self, ontology_table, feature_ontology_table):
        ...
        for agent_slice in agents_slices_pb2.agents_slices:
            agents_list = AGENT_REGISTRY[self.requested_agent_type].load(
                agent_slice.agent_snapshots, agent_ontology, feature_ontology_table
            )
            self.sample_id_to_agent_snapshots[agent_slice.slice_id.index] = agents_list

        for track in agent_tracks_pb2.agent_tracks:
            self.instance_id_to_agent_snapshots[track.instance_id] = AGENT_REGISTRY[self.requested_agent_type].load(
                track.agent_snapshots, agent_ontology, feature_ontology_table
            )
...
class AgentDataset(BaseAgentDataset):
    ...
    def _build_agent_index(self):
        ...
        for index in range(len(self.dataset_item_index)):
            ...
            if main_agent_id_and_scene_idx not in self.dataset_agent_index:
                self.dataset_agent_index[main_agent_id_and_scene_idx] = [-1, -1, math.inf, -1, []]
    ...
    def _build_item_index(self):
        ...
        with Pool(cpu_count()) as proc:
            item_index = proc.starmap(self._item_index_for_scene,
                                      [(scene_idx,) for scene_idx in range(len(self.agent_groups))])
    ...
    def __getitem__(self, index):
        ...
        for qsample_idx_in_scene in range(sample_idx_in_scene_start, sample_idx_in_scene_end + 1):
            ...
            instance_matched = [
                agent.instance_id == main_agent_id for agent in agents_in_sample
            ]
    ...
    def from_agent_containers(cls, agent_containers, requested_agent_types=None, requested_feature_types=None):
        ...
        if not requested_agent_types or not requested_feature_types:
            ...
            return cls(agent_containers, directory=os.path.dirname(agent_containers[0].directory),
                       feature_ontology_table={}, ontology_table={})
        feature_ontology_table = {}
        ...
        unique_scenes = {
            ...
            for _, _, filenames in os.walk(os.path.join(agent_container.directory, FEATURE_ONTOLOGY_FOLDER)) for f in
            filenames
        }
