...
def render_agent_bev(agent_dataset, ego_dimensions, render_pointcloud):
    ...
    for k in tqdm(range(0, len(agent_dataset))):
        ...
        img = visualize_agent_bev(agents, [lidar], render_pointcloud, class_colormap, show_instance_id_on_bev=False, id_to_name = id_to_name, bev_font_scale = .5, bev_line_thickness = 2\
...
if __name__ == '__main__':
    ...
    parser.add_argument('--render_pointcloud', default=False)
    ...
    bev_frames = render_agent_bev(agent_dataset_frame, ego_vehicle, args.render_pointcloud)
