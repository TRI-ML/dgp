...
def calc_warp_pose(pose_other, pose_target):
    # return pose for going from pose to pose_target
...
def render_agent_bev(
    ...
    tvec = np.array([vehicle_length / 2 - vehicle_applanix_origin_to_r_bumper, 0, 0])
    ego_box = BoundingBox3D(
        Pose(tvec=tvec), sizes=np.array([vehicle_width, vehicle_length, vehicle_height]), class_id=1, instance_id=0
    )
    ...
    for k in tqdm(range(0, len(agent_dataset))):
        ...
        img = visualize_bev([lidar], class_colormap, show_instance_id_on_bev=False, id_to_name = id_to_name, bev_font_scale = .5, bev_line_thickness = 2\
...
if __name__ == '__main__':
    ...
    agent_dataset_lite = AgentDatasetLite(
        args.scene_dataset_json,
        args.agent_json,
        split=args.split,
        datum_names=['lidar', 'CAMERA_01'],
